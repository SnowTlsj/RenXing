...
cron "5 2-23/4 * * *" me_try3.js, tag:京东试用3
 */
...
!(async() => {
    ...
    if (1) {
        ...
        let ONE1=Math.ceil($.cookiesArr.length/3)*2;
        let ONE2=Math.ceil($.cookiesArr.length/3)*3;

        if (ONE2>$.cookiesArr.length){
           ONE2=$.cookiesArr.length;
           }

        console.log(`本次执行第${ONE1+1}到${ONE2}个账号\n`);
        args_xh.tabId = args_xh.tabId.sort(() => 0.5 - Math.random())
        for (let i = ONE1; i < ONE2; i++) {
            if ($.cookiesArr[i]) {
                $.cookie = $.cookiesArr[i];
